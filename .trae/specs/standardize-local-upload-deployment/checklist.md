...
- [x] `deploy_server.py` 在上传前自动备份服务器端 `client/dist`、`server/dist`、`admin/server/dist`、`admin/client/dist`。
...
- [x] `.trae/skills/server-deploy-checklist/SKILL.md` 核心理念为本地构建产物、本地上传产物、服务器端仅替换/重启。
- [x] `.trae/skills/server-deploy-checklist/SKILL.md` 不再要求上传源码或在服务器端执行 `npx tsc` / `npm run build`。
- [x] `.trae/skills/server-deploy-checklist/SKILL.md` 中备份目录包含 `admin/client/dist`。
- [x] `.trae/skills/server-deploy-checklist/SKILL.md` 中服务器 IP 使用占位符 `<DEPLOY_HOST>` 或从 `.env.deploy` 读取，无硬编码 IP。
- [x] `project_memory.md` 与 `.trae/skills/deploy-assistant/SKILL.md` 中备份目录描述包含 `admin/client/dist`。
- [x] 项目 `.trae/hooks.json` 的 `pre-task` / `pre-completion` / `pre-commit` 中备份目录提醒包含 `admin/client/dist`。
