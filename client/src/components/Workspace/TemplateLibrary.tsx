 ...
 * 3. 创建完成后自动为包含预置问题的节点写入预设问答对话（不再调用 AI，无进度弹窗）
 *
 * 交互细节：
 * - 全屏遮罩层，点击遮罩关闭
 * - 弹窗居中显示，深色背景，圆角
...
const TemplateLibrary: React.FC<TemplateLibraryProps> = ({ isOpen, onClose }) => {
   ...
   * 成功且模板中包含预置问题时，触发批量写入预设问答对话（不阻塞，不展示进度），
   * 随后直接关闭弹窗。
   * 创建失败（返回空 rootId 或抛出异常）时通过 Toast 提示用户并关闭弹窗。
   *
  ...
  const handleSelectTemplate = useCallback(async (template: TemplateData): Promise<void> => {
    try {
      ...
      // 创建失败：rootId 为空时提示用户并关闭弹窗
      if (!result.rootId) {
        useToastStore.getState().addToast('error', t('templateCreateFailed'));
        onClose();
        return;
      }
      ...
      if (nodesWithPresetCount > 0) {
        // 有预置问题，触发批量写入预设问答对话（不 await，不阻塞弹窗关闭）
        const { createPresetConversationsForTemplate } = useAppStore.getState();
        void createPresetConversationsForTemplate(
          result.nodeIds,
          result.template,
        );
      }
      ...
      // 异常处理：记录日志并通过 Toast 提示用户，同时关闭弹窗
