  ...
  const modalContent = (
    <>
      <div className={`flex items-center justify-between px-6 py-4 border-b border-dark-700 ${isMobile ? 'h-14' : ''}`}>
        ...
        <button
          onClick={onClose}
          className="p-1 text-dark-400 hover:text-white rounded-lg hover:bg-dark-700 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className={`p-6 ${isMobile ? 'flex-1 overflow-y-auto' : 'max-h-96 overflow-y-auto'}`}>
        {activeTab === 'info' && (
                  ...
                  <div className="p-4 bg-dark-800 rounded-xl">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="p-2 text-dark-400 hover:text-white rounded-lg hover:bg-dark-700 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
                        title="复制邀请码"
                      >
              ...
              >
                ...
                {isOwner && member.role !== 'owner' && (
                  <button
                    ...
                    disabled={isLoading}
                    className="p-1.5 text-dark-400 hover:text-red-400 rounded-lg hover:bg-dark-700 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
                    title="移除成员"
                  >
        ...
        {error && (
          <div className="mt-4 p-3 bg-red-900/30 border border-red-700 rounded-lg text-red-400 text-sm flex items-center justify-between">
            <span>{error}</span>
            <button onClick={clearError} className="text-red-400 hover:text-red-300 min-w-[44px] min-h-[44px] flex items-center justify-center">×</button>
          </div>
        )}
      </div>
    </>
